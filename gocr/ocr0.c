...
//  ToDo: because we do modifications here, call it with a copy!?
//        modification can have undesired side effects else
//  dont remove upper dot from ":" 2010-09-30 
int testumlaut(struct box *box1, int cs, int m, wchar_t *modifier){
   ...
   if (box1->num_frames<1) return 0;
   if (box1->num_frames==2) {
     if (box1->y0>box1->m1
      &&  abs(box1->frame_vol[0]
             -box1->frame_vol[1])
        <=abs(box1->frame_vol[0]
             +box1->frame_vol[1])/8) return 0; // ":" 2010-09-30
   }
   r=0;
   ...
   if (y0 >= m2 && y1 >= m3) return 0; // 2010-10-08 ocr-a + ocr-b + qemu ';'
...
// 2010-09-24 this was bad code, destroying earlier good work for ":"
...
#if 0
     // temporary set new y0 (not needed!)
...
// out_x(box1);printf(" x,y=%d,%d i=%d xl=%d xr=%d yu=%d yl=%d",x0,y0,i-x0,xl-x0,xr-x0,yu-y0,yl-y0);
	   mod = DIAERESIS;
         ...
         // ToDo: bad work? use vector code or extra subbox outside! 2010-09
         if ((m&2) && box1->y0!=yl) {
           MSG(fprintf(stderr,"set new upper bound y0+= %+3d",yl-box1->y0);)
           box1->y0=yl;
         }
...
//           > loop(box1->p,xr,yl,xr-xl,cs,0,LE) ) // +dx/8 ) &agrave; Nov03
     ...
     fprintf(stderr," yu=%d yl=%d xl=%d xr=%d",yu-y0,yl-y0,xl-x0,xr-x0);
	   ...
	   mod = DOT_ABOVE; // "." above "ij", not ":;", ToDo: improve it!
         }
...
static wchar_t ocr0_eE(ocr0_shared_t *sdata){
   ...
   // new vector based fat e variant withot holes 2010-10-10
   // --- test e ---------------------------------------------------
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4 (smallest seen is 5x6)
      DBG( wchar_t c_ask='e'; )
      if (sdata->holes.num > 0)  break;
      if (box1->num_frames != 1) break;
/* 7x7
  ...
  .@@@@@@<- 
*/
      if (aa[0][0]>x0+dx/4 || aa[0][1]>y0+dy/4) Break;
      if (aa[1][0]>x0+dx/4 || aa[1][1]<y1-dy/4) Break;
      if (aa[2][0]<x1-dx/4 || aa[2][1]<y1-dy/4) Break;
      if (aa[3][0]<x1-dx/4 || aa[3][1]>y0+dy/4) Break;
      // upper body must at least 2 times thicker than low line
      i= loop(box1->p,x0+dx/2,y0  ,y1-y0,cs,0,DO); if (i>dy/8) Break;
      i= loop(box1->p,x0+dx/2,y0+i,y1-y0,cs,1,DO);
      j= loop(box1->p,x0+dx/2,y1  ,y1-y0,cs,0,UP); if (j>dy/8) Break;
      j= loop(box1->p,x0+dx/2,y1-j,y1-y0,cs,1,UP); if (j>dy/3) Break;
      if (i<2*j-dy/16) Break;
      // leftmost gap from the right
      i1=nearest_frame_vector(box1,aa[2][3],aa[3][3], x0, y0+2*dy/3);
      if (box1->frame_vector[i1][0]>=x0+dx/3) Break;
      if (box1->frame_vector[i1][1]<=y0+dy/2) Break; // strong for bad e
      i2=nearest_frame_vector(box1,i1      ,aa[3][3], x1, y0+2*dy/3);
      if (box1->frame_vector[i2][0]< x1-dx/8-1) Break;
      if (box1->frame_vector[i2][1]< y0+dy/2-1) Break; // strong for bad e
      if (box1->m2) {
...
static wchar_t ocr0_M(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>3 && dy>3;){	// dy<=dx nicht perfekt! besser mittleres
      ...
      if (loop(bp,  0,dy-1, dx,cs,0,RI)>dy/4) Break; // ~V 2010-10
...
static wchar_t ocr0_N(ocr0_shared_t *sdata){
   struct box *box1=sdata->box1;
   int	i,d,x,y,i1,i2,i3,hchar=sdata->hchar,gchar=sdata->gchar,
   ...
   for(ad=d=100;dx>3 && dy>3;){  // 4x6font
      ...
      if (aa[1][1]<=y1-dy/4) Break; // joined ty
      if (aa[1][0]>x0+dx/8) Break; // 2010-10-11
      if (abs(aa[3][1]-aa[0][1])>(dy+2)/5) Break; /* glued tu */
      if (abs(aa[3][1]-aa[0][1])>(dy+4)/8) ad=98*ad/100; /* glued tu */
      i= nearest_frame_vector(box1,aa[2][3],aa[3][3], x0, y0+dy/2);
      x=box1->frame_vector[i][0];
      if (x<=x0+dx/2 || x<aa[2][0]-dx/8) Break; // ~k 2010-10
...
*/
     ...
     i3=nearest_frame_vector(box1, i2, i4, (x0+x1)/2,        y1);
     // get the leftmost point between right up and down line
     i =nearest_frame_vector(box1, i5, i6,  x0-dx, (y0+2*y1)/3);
     ...
     if (x1-box1->frame_vector[i][0]<dx/2) { ad=99*ad/100; MSG({}) }
      ...
      /* check for falling line in the lower right corner */
      for (j=x=0,y=5*dy/8;y<7*dy/8;y++) {
        i= loop(bp,dx-1,y,dx,cs,0,LE); if(i>x) { x=i;j=y; }
      } // x=dx/6 on fat k
      if (x + loop(bp,dx-1-x,j,dx,cs,1,LE)/2 <dx/4) Break;  // y to j 2010-09
 ...
 */
      ...
      i=loop(bp,   0,dy-1-dy/8,dx,cs,0,RI); // 2010-10 -2 to -1
      j=loop(bp,dx-1,dy-1-dy/8,dx,cs,0,LE); // -//-
...
static wchar_t ocr0_vV(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
      DBG( wchar_t c_ask='v'; )
      if (sdata->holes.num > 0) Break; /* no tolerant against a tiny hole */
/* V.8x10
 ...
 */
      ...
      if (!hchar // 2010-10
       && loop(bp,0   ,dy/6     ,dx,cs,0,RI)
        >=loop(bp,0   ,dy-1-dy/3,dx,cs,0,RI) && dy>6 ) Break;
      if ( hchar
       && loop(bp,0   ,     dy/3     ,dx,cs,0,RI)
        >=loop(bp,0   ,dy-1-dy/3     ,dx,cs,0,RI)
       && loop(bp,0   ,dy-1-dy/3     ,dx,cs,0,RI)
        >=loop(bp,0   ,dy-1-dy/3+dy/6,dx,cs,0,RI) && dy>6 ) Break; // 2010-10
...
static wchar_t ocr0_rR(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dy>3 && dx>1;){  // dy>dx, 4x6 font, dx=2 smallest prop-font
      ...
      // 2010-10 ocr-a-subset 4x8 '' was detected as bad flying r
      i1=nearest_frame_vector(box1,aa[3][3],aa[0][3], x0+dx/3, y0+dy/4);
      // i2=nearest_frame_vector(box1,aa[3][3],aa[0][3], x0+dx/3, y0+dy/4);
      if (box1->m2
       && 2*box1->y0 <= box1->m1 +   box1->m2
       && 3*box1->y1 <= box1->m2 + 2*box1->m3
       && box1->frame_vector[i1][1]-y0==0)
...
static wchar_t ocr0_tT(ocr0_shared_t *sdata){
   struct box *box1=sdata->box1;
   pix *bp=sdata->bp;
   int	i,i1,i2,i3,i4,j,d,d2,x,y,yb,hchar=sdata->hchar,gchar=sdata->gchar,
        x0=box1->x0,x1=box1->x1,y0=box1->y0,y1=box1->y1,cs=sdata->cs;
   int (*aa)[4]=sdata->aa;    /* corner-points, (x,y,dist^2,vector_idx) */
   int  dx=x1-x0+1,dy=y1-y0+1,	/* size */
        ad;	/* tmp-vars */
   ...
   for(ad=d=100;dx>2 && dy>3;){     // dx>1 dy>2*dx 
      ...
      if (box1->num_frames>1){
        if (4*box1->frame_vol[1]<box1->frame_vol[0]) Break; // ~?
        ad=ad*99/100; // ~? ocr-b.png 2010-10-07
      }
      ...
      // top width
      for( d2=0,y=0;y<=dy/4;y++){
        i=loop(bp,0,y,dx,cs,0,RI); // possible gap?
        j=loop(bp,i,y,dx,cs,1,RI); // width of horizontal line
        MSG(fprintf(stderr,"ij1 %2d %2d",i,j);)
        if (dy>16 && y>0 && i+j<dx) j+=loop(bp,i+j,y-1,dx-i-j,cs,1,RI);
        if (dy>32 && y>1 && i+j<dx) j+=loop(bp,i+j,y-2,dx-i-j,cs,1,RI);
        MSG(fprintf(stderr,"ij2 %2d %2d",i,j);)
        if (j>d2) d2=j; // found
      } if (3*d2<dx) Break; // 2010-10 ocr-b '?'
      if (4*d2<2*dx) {ad=99*ad/100;MSG({}) }  // handwritten T 2010-10
      if (5*d2<3*dx) {ad=99*ad/100;MSG({}) }  // handwritten T 2010-10
      // center width
   ...
   for(ad=d=100;dx>1 && dy>=box1->m3-box1->m2-1;){  // sometimes no hchar!
     ...
     if( num_cross(0,dx-1,3*dy/4,  3*dy/4,  bp,cs) >= 2
      ...
      // 2010-10 $ (S + vert. lines at bottom and top, not crossing the S)
      i1=nearest_frame_vector(box1, aa[0][3], aa[1][3], x1+2*dx, (y0+y1)/2);  
      i2=nearest_frame_vector(box1, aa[2][3], aa[3][3], x0-2*dx, (y0+y1)/2);  
      if (box1->frame_vector[i1][0]-x0>3*dx/4
       && box1->frame_vector[i2][0]-x0<  dx/4
       && box1->frame_vector[i1][1]-y0
         >box1->frame_vector[i2][1]-y0) Break;
      ...
      if (dx<3) {
        ad=ad*99/100; // ~ 2x11 )
        if (loop(bp,0,   0,dx,cs,0,RI)==0
         && loop(bp,0,dy-1,dx,cs,0,RI)==0
         && loop(bp,dx-1,(dy+1)/2,dx,cs,0,LE)==0) Break; // )
      }
      if (loop(bp,0,   0,dx,cs,0,RI)<=dx/8  // 2010-09-26
       && loop(bp,dx-1,dy/2+1,dx,cs,0,LE)<=dx/8) {
         ad=ad*98/100; // ~) 3x11 
        MSG(fprintf(stderr,"ad= %d",ad);)
      }
...
static wchar_t ocr0_sS(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4 (4x6 font)
      ...
      if( num_cross(  dx/2,  dx/2,0,dy-1,bp,cs)!=3
       && num_cross(6*dx/8,2*dx/8,0,dy-1,bp,cs)!=3 // 6x7 fat s 2010-09-25
       && dy>4 ) Break;
      if( num_cross(0,dx-1,dy/2  ,dy/2  ,bp,cs)!=1
       && num_cross(0,dx-1,dy/2-1,dy/2-1,bp,cs)!=1 ) Break;
      // get the upper and lower hole koords
      y=(dy+2)/4; // 6x7 fat s 2010-09-25
      x  =loop(bp,0,(dy+2)/4,dx,cs,0,RI); if(x>3*dx/8) Break; /* slanted too */
      x +=loop(bp,x,(dy+2)/4,dx,cs,1,RI); if(x>5*dx/8) Break; /* fat too */
      i1 =loop(bp,x,(dy+2)/4,dx,cs,0,RI); i1=(i1+2*x)/2; // upper center x
      y=11*dy/16; // not safe for 6x7 fat font
      ...
      i3  =loop(bp,0,(dy+2)/4,dx,cs,0,RI);
...
//      }
      ...
      if (!joined(bp,0,y,i2,11*dy/16,cs)) Break; // ? 6x7 fat s tmp08/gocr0801_bad5

      if (sdata->holes.num > 0)
      if (sdata->holes.hole[0].y0 > (dy+2)/4) Break; // ???
      // if( num_hole( x0, x1, y0+dy/4, y1, box1->p,cs,NULL) > 0 ) Break;
...
static wchar_t ocr0_xX(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>3;){     // min 3x4
      ...
      // JS-2010-09 tmp08/0811qemu2_crop (left top above right top?)
      if (box1->m2-box1->m1>2 &&
       3*(aa[3][1]-aa[0][1])>=2*(box1->m2-box1->m1)) Break; // ~k

      /* 2009-07: false positive on font4x5 '*' = '-' | 'x' */
      /* 2010-08: +/-(0,dy/4) to +/-(dx/4,dy/3) (handwritten x) */
      i1=nearest_frame_vector(box1,aa[0][3],aa[1][3],x0+dx,  y0+dy/3);
      i3=nearest_frame_vector(box1,aa[0][3],aa[1][3],x0+dx,  y1-dy/3);
      i2=nearest_frame_vector(box1,i1,      i3,      x0+dx/4,y0+dy/2);
      ...
      i1=nearest_frame_vector(box1,aa[2][3],aa[3][3],x0     ,y0+dy/3);
      i3=nearest_frame_vector(box1,aa[2][3],aa[3][3],x0     ,y1-dy/3);
      i2=nearest_frame_vector(box1,i1,      i3,      x1-dx/4,y0+dy/2);
...
static wchar_t ocr0_zZ(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>3 && dy>3;){     // dy>dx
      ...
      if (sdata->holes.num > 0) Break;
      if (box1->num_frames > 1) ad=98*ad/100; /* tolerant, tiny hole */
      /* half distance to the center */
      d=2*sq(128/4);
      ...
      // 2010-10-11 ~ joined ty (tmp08/gocr0801_bad5)
      // check bottom line to be low
      i= nearest_frame_vector(box1,aa[1][3],aa[2][3], x0+dx/2, y0);
      j= box1->frame_vector[i][1];
      if (j<y0+dy/2
       || (j<aa[1][1]-dy/8
        && j<aa[2][1]-dy/8)) Break;
...
static wchar_t ocr0_wW(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>3 && dy>3;){     // dy<=dx
     ...
     if (ya<dy/2 && xc<  dx/2) ad=98*ad/100; /* ~N */
     if (ya<dy/2 && xc<3*dx/8) ad=96*ad/100; /* ~N */
...
// ToDo: MSG(fprintf(stderr,"x,y,i4,i3= %d %d %d %d",x,y,i4,i3);)
   ...
   // new vector based fat a variant withot holes 2010-10-11
   // --- test a ---------------------------------------------------
   for(ad=d=100;dx>2 && dy>4;){     // min 3x5
      DBG( wchar_t c_ask='a'; )
      if (sdata->holes.num > 1)  break;
      if (box1->num_frames != 2 + box1->dots ) break;
/* tmp08/gocr0801_bad5
   7x7        7x7qemu
...
*/
      ...
      if (aa[2][0]<x1-dx/4 || aa[2][1]<y1-dy/4) Break;
      if (aa[3][0]<x1-dx/4 || aa[3][1]>y0+dy/4) Break;
      // ToDo: check position of 2nd frame (hole) using nearest_x and _y???
      // rightmost gap from the left
      i1=nearest_frame_vector(box1,aa[0][3],aa[1][3], x1, y0+1*dy/3);
      if (box1->frame_vector[i1][0]<=aa[0][0]
       || box1->frame_vector[i1][0]<=aa[1][0]) Break; // must!
      if (box1->frame_vector[i1][0]< x0+dx/3) Break;  // very tolerant
      if (box1->frame_vector[i1][0]< x1-dx/3) {
        // we allow one exeption where fat lines close the gap (see tmp08)
        // upper body must at least 2 times thicker than low line
        i= loop(box1->p,x0+dx/2,y0  ,y1-y0,cs,0,DO); if (i>dy/8) Break;
        i= loop(box1->p,x0+dx/2,y0+i,y1-y0,cs,1,DO);
        j= loop(box1->p,x0+dx/2,y1  ,y1-y0,cs,0,UP); if (j>dy/8) Break;
      ...
      if (box1->frame_vector[i1][1]>y0+dy/2) Break;
      i2=nearest_frame_vector(box1,i1      ,aa[1][3], x0-dx/2, y0+1*dy/2);
      MSG(fprintf(stderr,"i12 %d %d y=%d %d", i1, i2, box1->frame_vector[i2][1]-y0, y1-dy/4-1-y0 );)
      if (box1->frame_vector[i2][0]> x0+(dx+4)/8+1) Break;
      if (box1->frame_vector[i2][1]> y1-dy/4) Break; // ?
      for (i=0;i<box1->num_frames;i++)
        if (box1->frame_vol[i]<0) break;
      if (i>=box1->num_frames || i<1) Break; // no hole? hole only?
      i3= box1->num_frame_vectors[i-1];
      i4= box1->num_frame_vectors[i]-1;
      MSG(fprintf(stderr,"i34 %d %d = hole",i3,i4);)
      i5= nearest_frame_vector(box1, i3, i4, x0+dx/2, y0);
      MSG(fprintf(stderr,"i5 %d y %d",i5,box1->frame_vector[i5][1]-y0);)
      if (box1->frame_vector[i5][1]<y0+dy/3) Break;
   ...
   for(ad=d=100;dx>3 && dy>3;){     // min 4x4
      ...
      if (ad>99
       && loop(bp,dx-1,dy-1,y1-y0,cs,0,UP)> dy/32 ) ad=98*ad/100; // ~o 2010

      if (hchar) ad=96*ad/100; // 2010-10 ~O
      if (gchar) ad=98*ad/100;
      // handwritten-a (alpha)
      Setac(box1,'a',ad);
      break;
...
static wchar_t ocr0_lL(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>2 && dy>4;){     // min 3x4
      ...
      // tmp08/gocr0801_bad5_FP.jpg 6x9 1  2010-09-25
      if (loop(bp,0,dy-2,dx,cs,0,RI)
         -loop(bp,0,dy-1,dx,cs,0,RI)>dx/4) Break; // ~l1 6x9 font
   ...
   for (ad=d=100; dy>dx && dy>5;) {     // min 3x4
      ...
      if (box1->num_frames>1 && box1->m3) Break; // ~! 2010-10-01
/* 2010-09-24 6x10 1 similar but not ~l sample gocr_codabar.png
  6x10=1=~l  6x9=1=~l 
  ..@@..<- 
  .@@@..       @@@...<-
  ..@@..       @.@...
 ...
 */       
      ...
      if ( loop(bp,   0,dy/2,dx,cs,0,RI)>=dx/2
        && (i<dx/2 || i==0) && dx>1) ad=98*ad/100; // ~] JS-2010-09 add dx>1
      if ( get_bw(x0,x0,y0  ,y1  ,box1->p,cs,2) != 2
        && get_bw(x0,x1,y0  ,y0  ,box1->p,cs,2) != 2
        && get_bw(x0,x1,y1  ,y1  ,box1->p,cs,2) != 2
...
//      if( vertical_detected && dx>5 )
      ...
      if ( dx>1 &&  // important for dx/2 below
             ( loop(bp,dx-1,   0,dx,cs,0,LE)<=dx/8
      ...
      i=loop(bp,0,0,dy,cs,0,DO);
      if (3*i>dy && 3*i<2*dy
       && loop(bp,dx-1,   0,dy,cs,0,DO)==0
       && loop(bp,dx-1,dy-1,dy,cs,0,UP)==0) {
        i=loop(bp,   0,dy-1,dy,cs,0,UP);
        if (3*i>dy && 3*i<2*dy) Break; // ~{} 6x10
        ad=ad*99/100;
      }

      // JS-2010-09 badly implemented for (dx=1)-"l"?
      //MSG( fprintf(stderr,"ad= %d", ad); )
      //if ( get_bw(x0,x0,y1,y1,box1->p,cs,2) == 0 ) ad=99*ad/100;
      //if ( get_bw(x1,x1,y1,y1,box1->p,cs,2) == 0 ) ad=99*ad/100;
      if (ad==100) ad--; /* I have to fix that: (sample?)
      ...
      */
      ...
      if(!hchar) ad=ad*99/100;
      if( gchar) ad=ad*99/100;
      // full rectangle? no white dots? I and l should have 98% and 99%
      //  after context correction (JS2010-09)
      if (ad>98 && get_bw(x0,x1,y0,y1,box1->p,cs,2) == 0) ad=99*ad/100;
      // 2010-10-01 sample tmp10/barcode_code128_145      
      if (dx<3 && dy>10 && box1->m4==0) ad=ad*95/100; // just a vertical line?
...
//     && ( get_bw(x0,x0+dx/32,y0,y0+dy/32,box1->p,cs,1) == 1
      ...
      if (ad==100 && bc=='o' && box1->m2
       && abs((box1->m2 - box1->y0)
            - (box1->y0 - box1->m1))
           <= (box1->m2 - box1->m1)/4) ad=98*ad/100; // unsure 2010-10-01
      DBG( c_ask=bc; )
      Setac(box1,bc,ad);
      DBG( c_ask='0'; )
...
static wchar_t ocr0_iIjJ(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dy>3 && dx>0;){     // min 3x4 without dot
      DBG( wchar_t c_ask='i'; )
      if (sdata->holes.num > 1) Break; /* tolerant against a tiny hole */
      if (box1->num_frames > 2) Break; /* ~% , ToDo: allow double dot i? */
      ...
      if (sdata->holes.num>0) ad=96*ad/100; // ~ fat italic a gocr0801_bad5
...
//    if( box1->dots!=1 ) Break;
      ...
      // ya includes possible i-dot
      // failed on handwritten i m1=-22 frame2y=-31..-24 dy=15
      // volume periphery(de:Umfang) num_vectors
      //#  frame 0 ( +52, 32,14) 3*vol1<2*vol0 2*dy1<dy0
      //#  frame 1 ( +28, 18, 8)
      // Aug2010
      if (box1->num_frames>1) { // vector based i-dot check
        if (box1->frame_vol[1]>box1->frame_vol[0]/8) // no dust
        { 
          if (3*box1->frame_vol[1]>2*box1->frame_vol[0]) Break; // to big
          if (3*box1->frame_per[1]>2*box1->frame_per[0]) Break; // not compact
          y=0;
          for (j=box1->num_frame_vectors[0];
               j<box1->num_frame_vectors[1] && j<MaxFrameVectors; j++) {
            if ( ya > box1->frame_vector[j][1]) // find top
                 ya = box1->frame_vector[j][1];
            if ( y  < box1->frame_vector[j][1]) // find bottom
                 y  = box1->frame_vector[j][1];
          }
          if (box1->m2 && ya>box1->m2+2) Break; // dot starts below m2
          if (2*y>=ya+y1) Break;		// point ends to low
        }
      } else { Break; /* missing i-dot */ }
//      out_x(box1);
#if 0 // dont work, because dots are removed as neighbours Aug10
      /* pixel based i-dot check */
      // search upper end of i-dot
      for (y=ya;3*y<ya+2*y1;y++)
        if( get_bw(x0,x1,y,y,box1->p,cs,1) == 1 ) break;
      if (3*y>=ya+2*y1) Break;		// hmm, gap only, no dot?
...
// handwritten: ya=-31 m1=-22 yb=0 y1=15
//  may be its better to trust the frame melting algorithm (ad=96%=ok)
      if (5*yb>=1*ya+4*y1) Break;		// huge gap, ~:
      if (loop(bp,dx-1,yb+(y1-ya+1)/32,dx,cs,0,LE)>dx/2) // unusual (right part of ouml)
        ad=95*ad/100;
   ...
   for(ad=d=100;dy>4 && dy>dx && 5*dy>4*(box1->m3-box1->m2);){     // min 3x4
      ...
      if ( loop(bp,dx-1,  dy/2,dx,cs,0,LE) <= dx/8
        && loop(bp,   0,  dy/2,dx,cs,0,RI) >  dx/2 ) Break; /* ~] 2010-10 */
      if ( loop(bp,dx-1,  dy/2,dx,cs,0,LE) >  dx/2
        && loop(bp,   0,  dy/2,dx,cs,0,RI) <= dx/8 ) Break; /* ~[ 2010-10 */
      ...
      if (loop(bp,dx-1,0,dx,cs,0,LE)                     // tmp08/0811qemu1
         -loop(bp,   0,0,dx,cs,0,RI)==dx/4) ad=98*ad/100; // ~l 4x10 2010-09

      // JS-2010-09 do we have a full square (bad font) we can heve I or l
      // I and l should have 98% and 99% after context correction (JS2010-09)
      if (ad>98 && get_bw(x0,x1,y0,y1,box1->p,cs,2) == 0)
...
static wchar_t ocr0_brackets(ocr0_shared_t *sdata){
   ...
   for(ad=d=100;dx>1 && dy>2;){     // min 3x2
      ...
      for (i1=i=aa[0][3];i!=aa[1][3];i=(i+1)%box1->num_frame_vectors[0]) {
        if (box1->frame_vector[i ][0]
          >=box1->frame_vector[i1][0]) i1=i; /* notice most right vector */
      } if (i1==i || i1==aa[0][3]) Break;
      /* calculate the distance to the center */
      x=box1->frame_vector[i1][0];
      y=box1->frame_vector[i1][1];
      ...
      if (    aa[0][0]+aa[1][0]-2*x>=0) Break;
      d=line_deviation(box1, aa[0][3], i1) >sq(1024/4);
      /* check if upper left and center point are joined directly */
      MSG(fprintf(stderr,"x %d %d dist= %d/%d",x-x0,y-y0,d,sq(1024/4));)
      if (d >sq(1024/4)) Break;  ad=ad-d*100/sq(1024);
      MSG(fprintf(stderr,"ad=%d", ad);)
      j=nearest_frame_vector(box1,i1,aa[1][3],x0-2*dx,y1-dy/8-1);
      d=line_deviation(box1, i1, j /*aa[1][3] */); // 2010-10 ocr-b
      /* check if lower left and center point are joined directly */
      MSG(fprintf(stderr,"x %d %d dist= %d/%d",x-x0,y-y0,d,sq(1024/4));)
      if (d >sq(1024/4)) Break;  ad=ad-d*100/sq(1024);
      MSG(fprintf(stderr,"ad=%d", ad);)
      ...
      for (j=i2=i=aa[1][3];i!=aa[0][3];i=(i+1)%box1->num_frame_vectors[0]) {
        if (box1->frame_vector[i ][0]
          >=box1->frame_vector[i2][0]) i2=i; /* notice most right vector */
          // MSG(fprintf(stderr,"search right: %d %d %d %d",i,i2,aa[1][3],aa[0][3]);)
      } if (i2==i || i2==aa[1][3]) Break;
      /* calculate the distance to the center */
      x=box1->frame_vector[i2][0];
      y=box1->frame_vector[i2][1];
      if (   (aa[0][0]+aa[1][0]-2*x)>=   0   ) Break;
      ...
      j=nearest_frame_vector(box1,i2,aa[0][3],x0+dx/4,y0-2*dy);
      if (box1->frame_vector[j][0]-x0>dx/4) Break;
      d=line_deviation(box1, i2, j /* aa[0][3] */);
      /* check if upper left and center point are directly joined directly */
      MSG(fprintf(stderr,"x %d %d dist= %d/%d",x-x0,y-y0,d,sq(1024/4));)
      if (d >sq(1024/4)) Break;  ad=ad-d*100/sq(1024);
      MSG(fprintf(stderr,"ad=%d", ad);)

      j=nearest_frame_vector(box1,aa[1][3],i2,x0+dx/4,y1+2*dy);
      if (box1->frame_vector[j][0]-x0>dx/4) Break;
      i=nearest_frame_vector(box1,aa[1][3],i2,x1+2*dx,y0+dy/2+dy/8);
      if (box1->frame_vector[i][1]-y0>dy/2+dy/8) Break; // ~()
      d=line_deviation(box1, j /* aa[1][3] */, i /* i2 */); // 2010-10-08
...
//   if(!box1->dots)
   for(ad=d=100;dx>3 && dy>3;){     // min 4x4 for 4x6 font
      ...
      i1=loop(bp,dx-1,dy/16,dx,cs,0,LE); // top    right end
      i3=loop(bp,dx-1,dy-1 ,dx,cs,0,LE); // bottom right end
      i4=loop(bp,   0,0    ,dx,cs,0,RI); // top    left  end
      i6=loop(bp,   0,dy-1 ,dx,cs,0,RI); // bottom left  end
      // i= (box1->m4 + box1->m3)/2 - box1->m2;
      // out_x(box1);printf("() %d %d %d %d %d %d %d\n",i,i1,i2,i3,i4,i5,i6);

      // ~lI     
      for(i=i4,y=0;y<dy;y++){  // check for left deviations from line
        x=loop(bp,0   ,y,dx,cs,0,RI);if(abs(x-i)>dx/6+1 ) break; i=x;
      } if( y<dy ) Break;
      for(i=i1,y=0;y<dy;y++){  // check for right deviations from line
        x=loop(bp,dx-1,y,dx,cs,0,LE);if(abs(x-i)>dx/6+1 ) break; i=x;
      } if( y<dy ) Break;
      
      if (box1->m2 &&  sdata->gchar) ad=99*ad/100; // 2010-10
      if (box1->m2 && !sdata->hchar) ad=98*ad/100;
      if (box1->m2 && dy < box1->m3 - box1->m2 -1) ad=96*ad/100; // ~,
      
      // check top-right-end, bottom-left-end
      if (i1<=dx/8  && i6<=dx/8  && i4-(dx-i3)>=dx/8 ) { // /
        if (i4<=dx/8 && i3<=dx/8) Break;
        if (i4-(dx-i3)<dx/4) ad=99*ad/100; // not enough slope
        Setac(box1,(bc='/'),ad);break;
      }
      if (i4<=dx/8  && i3<=dx/8  && i6-(dx-i1)>=dx/8 ) { // \ ...
        if (i6<=dx/8 && i1<=dx/8) Break;
        if (i6-(dx-i1)<dx/4) ad=99*ad/100; // not enough slope
        Setac(box1,(bc='\\'),ad); break;
      }
      Break;
   }
...
//      if(i2==0 && 3*i5>dx && i4<=dx/8 && i6<=dx/8) { Setac(box1,(bc='>'),98);{Break;}; }
      ...
      if( dx > 2 && 9*dx>=5*dy ){  // 4x6 screen-font (3*5)
       ...
       if( 2*dx > OCR_JOB->res.avX && 4*dx>dy ) ad=98;
// printf(" %d %d %d %d %d %d\n",i5,i1,i3,i2,i4,i6);
...
*/
   ...
   //  --------- test [] --------------------------------
   for(ad=d=100;dx>2 && dy>4 && dy>=2*dx;){ // (3,6) on 4x6 font
      DBG( wchar_t c_ask=']'; )
      if (sdata->holes.num > 1) { Break;} /* tolerant against a tiny hole */
      if (box1->num_frames!=1) break;
      if (box1->num_frame_vectors[0]!=10) ad=98; // not charp, unsure
      if (!hchar) ad=97*ad/100;
      for(y=0;y<dy;y++){
        if( num_cross(0,dx-1,y,y,bp,cs) != 1 ) break;
      } if (y<dy) {Break;};
   ...
   for(ad=d=99;dx>2 && dy>5 && 2*dy>3*dx;){
      ...
      i1=loop(bp, 0,dy/4,dx,cs,0,RI);
      i1=loop(bp,i1,dy/4,dx,cs,1,RI); // thickness1
      for (i2=dx,i3=y=dy/2-1-dy/16;y<dy/2+2+dy/16;y++)
        { x=loop(bp, 0, y ,dx,cs,0,RI); if  (x<i2) {i2=x;i3=y;} }
      i2=loop(bp,i2,i3,dx,cs,1,RI); // thickness2
      if (i2<i1+dx/16+1) Break;
      if ( loop(bp,0,dy-1,dx,cs,0,RI)>3*dx/4 ) {ad=99*ad/100;MSG({})}
      if ( loop(bp,0,   0,dx,cs,0,RI)>3*dx/4 ) {ad=99*ad/100;MSG({})} // <
      if ( loop(bp,0,   0,dy,cs,0,DO)<dy/2-dy/8-1 ) {ad=98*ad/100;MSG({})}
      if ( loop(bp,0,dy-1,dy,cs,0,UP)<dy/2-dy/8-1 ) {ad=98*ad/100;MSG({})} // (
      if ( loop(bp,0,dy-1,dy,cs,0,UP)<=dy/4 ) Break; // (
      if (dy>=8)
      if (   loop(bp,dx-1,   0,dx,cs,0,LE)
         +   loop(bp,dx-1,dy/4,dx,cs,0,LE)
         - 2*loop(bp,dx-1,dy/8,dx,cs,0,LE) >=dx/8 ) {ad=98*ad/100;MSG({})} // <
...
static wchar_t ocr0p9(ocr0_shared_t *sdata){
   ...
   // --- test $ (vert. line crossing) ------------------------------
   for(ad=d=99;dx>3 && dy>6;){     // min 4x7 2010-10
      DBG( wchar_t c_ask='$'; )
      if (sdata->holes.num != 2) Break;
      ...
      // upper left hole
      // MSG( fprintf(stderr,"hole[0]=x0=%d %d", sdata->holes.hole[0^i4].x0, sdata->holes.hole[0^i4].x1); )
   ...
   // --- test $ (no vert. line crossing, but lines on top and bottom) -----
   for(ad=d=99;dx>3 && dy>6;){     // min 4x7  2010-10
      DBG( wchar_t c_ask='$'; )
      if (box1->num_frames != 1) break;
/*  5x7
 ...
 */      
   ...
   if(bc==UNKNOWN)
   for(ad=d=99;dx>4 && dy>7;){ // 2010-09-24 5x8 sample
      DBG( wchar_t c_ask='@'; )
      if (sdata->holes.num > 3) Break; /* tolerant against a tiny hole */
      ...
      x=(7*dx+3)/16; // 8x10=3 7x9=3 5x8=2 (within vert. middle line)
      y=dy/2;
      i=num_cross(0,dx-1,y,y,bp,cs);
      j=loop(bp,   0,y,dx,cs,0,RI); // measure line width, 2010-09-24
      j=loop(bp,   j,y,dx,cs,1,RI);
      if (dx>=4*j && (i<3 || i>4)) Break;
      if (dx>=5*j && i!=4) ad=98*ad/100;
      ...
      if( num_cross(x,dx-1,y,y,bp,cs) != 2 && dx>=4*j) Break;
...
static wchar_t ocr0px(ocr0_shared_t *sdata){
   ...
   for(ad=99;dy>4 && 2*dx<dy;){     /* v0.44 */
      ...
      if (box1->m2 && 3*y1<  box1->m2+2*box1->m3) ad=95*ad/100;
      ac='|';
      if (!hchar) ad=98*ad/100;
      Setac(box1,ac,ad);
      break;
   }
   /* --- test | (vertical line with small gap, pipe symbol) --- */
   if (box1->num_frames == 2)
   for (ad=99;dy>6 && 2*dx<dy;){     /* 2010-10-08 qemu */
      DBG( wchar_t c_ask='|'; )
      if (8*abs(box1->frame_vol[0]-box1->frame_vol[1]) // 10 + 8
           >    box1->frame_vol[0]+box1->frame_vol[1] ) Break;
      if ( box1->frame_vol[0]+box1->frame_vol[1]
          < 7*(dx*dy)/8 ) Break;
      /* test if everything is filled black */
      if ( get_bw(x0,x1,y0+dy/2+0,y0-dy/2+0,box1->p,cs,1) != 0
        && get_bw(x0,x1,y0+dy/2-1,y0-dy/2-1,box1->p,cs,1) != 0 ) Break;
      /* more unsure if the borders are not exact */
      if (3*dx<dy) ad=98*ad/100;
      if (4*dx<dy) ad=99*ad/100;
      if (box1->m2 && 4*y1<  box1->m2+3*box1->m3) Break;
      if (box1->m2 && 4*y1<0*box1->m2+4*box1->m3) ad=95*ad/100;
      ac='|';
      if (!hchar) ad=98*ad/100;
      Setac(box1,ac,ad);
      break;
   ...
   for(ad=100;dx>5 && dy>7;){     // provisorium
      ...
      if ( box1->num_frames != 1
        && box1->num_frames != 3
        && box1->num_frames != 5 ) break;
      if (box1->num_frames == 1) { // tiny fat font?
        Break;
        if (dx>5) Break;
        ad=ad*95/100;
      }
      if (box1->num_frames >= 3
       && box1->frame_vol[1]<0
       && box1->frame_vol[2]<0) { // small font? 7x8
/*  12x17 ocr-b
   .@@@...@
   @@.@@.@@
   .@@@.@@.
   ...
   @...@@@.
*/
        // aa[] belongs to the frame 0 only! 2010-10
        if (aa[0][0]-x0>  dx/4 || aa[0][1]-y0>  dy/4) Break; // upper left  /
        if (aa[2][0]-x0<3*dx/4 || aa[2][1]-y0<3*dy/4) Break; // lower right /
        if (aa[1][0]-x0>  dx/4 || aa[1][1]-y0<3*dy/4) Break; // lower left  /
        if (aa[3][0]-x0<3*dx/4 || aa[3][1]-y0>  dy/4) Break; // upper right /
        j =nearest_frame_vector(box1,aa[3][3],aa[0][3],x0+dx/2,y0+dy/2);
        if (box1->frame_vector[j][0]-x0< dx/2
         || x1-box1->frame_vector[j][0]< dx/4
         || box1->frame_vector[j][1]-y0<=dy/8) Break; // ~8B
        j =nearest_frame_vector(box1,aa[1][3],aa[2][3],x0+dx/2,y0+dy/2);
        if (box1->frame_vector[j][0]-x0< dx/4
         || x1-box1->frame_vector[j][0]< dx/2
         || y1-box1->frame_vector[j][1]<=dy/8) Break; // ~8B
        if (dx>7 && ad==100) {ad=ad*99/100;MSG({})} // ocr-b
        for (i1=i2=1,i=1;i<box1->num_frames;i++) { // get biggest holes
          if (i1==i2 && i!=i1) i2=i; 
          if (box1->frame_vol[i ]<box1->frame_vol[i2] && i!=i1) i2=i; 
          if (box1->frame_vol[i2]<box1->frame_vol[i1]) { j=i1;i1=i2;i2=j; }
        } if (i1==i2) Break;
        if (abs(box1->frame_vol[i1])>=box1->frame_vol[0]/8) Break;
        if (abs(box1->frame_vol[i2])>=box1->frame_vol[0]/8) Break;
        if (abs(box1->frame_vol[i1] - box1->frame_vol[i2])   // vol=4
          >=abs(box1->frame_vol[i1] + box1->frame_vol[i2])/2) Break;
        // ToDo: check one box above and the other below dy/2 ("u)
        if (ad==100) {ad=ad*99/100;MSG(fprintf(stderr,"ad= %d",ad);)}
      }
      // MSG(fprintf(stderr,"aa2y dy %d %d",aa[2][1]-y0,dy);)
      if (box1->num_frames == 3
       && box1->frame_vol[1]>0) { // small font? 7x8
...
*/
        ...
        if (aa[3][0]-x0<3*dx/4 || aa[3][1]-y0>  dy/4) Break; // upper right /
        if (dx>7 && ad==100) {ad=ad*99/100;MSG({})} // ocr-b
        if (box1->frame_vol[1]>=box1->frame_vol[0]) Break;
        if (abs(box1->frame_vol[1] - box1->frame_vol[2])   // vol=4
          >=abs(box1->frame_vol[1] + box1->frame_vol[2])/8) Break;
        // ToDo: check one box above and the other below dy/2 ("u)
        if (ad==100) {ad=ad*99/100;MSG(fprintf(stderr,"ad= %d",ad);)}
      }
      if (box1->num_frames == 5
       && sdata->holes.num == 2
       &&  box1->frame_vol[1]>0) { // big font
        // aa[] belongs to the frame 0 only! 2010-10
        if (aa[0][0]-x0<  dx/4 && aa[0][1]-y0<  dy/4) Break; // upper left  /
        if (aa[2][0]-x0>3*dx/4 && aa[2][1]-y0>3*dy/4) Break; // lower right /
        if (aa[1][0]-x0>  dx/4 || aa[1][1]-y0<3*dy/4) Break; // lower left  /
        ...
        if (dx>7 && dy>12) {
          ...
          if( num_hole(x0     ,x1+dx/4,y0,y1     ,box1->p,cs,NULL) != 2 ) Break;
        } else {ad=98*ad/100;MSG({})}
      }

      // use box1->p instead of b, because % is a sum of 3 objects + 2 holes
      if (loop(box1->p,x0,y0        ,dx,cs,0,RI)
        < loop(box1->p,x0,y0+dy/16+1,dx,cs,0,RI)){ad=96*ad/100;MSG({})} // X
      if (loop(box1->p,x1,y1        ,dx,cs,0,LE)
        < loop(box1->p,x1,y1-1-dy/16,dx,cs,0,LE)){ad=96*ad/100;MSG({})} // X
...
wchar_t ocr0(struct box *box1, pix *bp, int cs){
   ...
   */
   ...
   // --- test thin lines = ---------------------------------
   for( ad=100; dy>2 && dx>2; ){     // min 3x3 (small font)
      DBG( wchar_t c_ask='='; )
      if (box1->num_frames != 2) break;
      ...
      if (2*dx <   dy) Break; // 2010-10-08
      if (4*dx < 3*dy) ad=ad*99/100;
      Setac(box1,'=',ad);
      return '=';
   }
   // --- test dots : ---------------------------------
   for( ad=100; dy>2 && dy>=2*dx; ){     // max 3x3 (small font)
      ...
      DBG( wchar_t c_ask=':'; )
      if (box1->num_boxes!=2 || box1->num_subboxes!=0) Break;
      // check the gap hight
   ...
   if (box1->num_frames == 2) // 2010-10-08
   for( ad=100; dy>5 && dx>1 && 2*dy>3*dx; ){     // max 3x3 (small font)
      DBG( wchar_t c_ask=';'; )
   ...
   if( 3*dy<2*(box1->m4-box1->m1)
    ...
    && box1->dots==0 ){  // ocr-a-,
      ...
      else { /* with upper filled circle or box */
        if( loop(box1->p,x0,(y0+y1+1)/2,x1-x0,cs,0,RI)<dx/2 ) ad=98*ad/100;
        if( loop(box1->p,x1,    y1     ,x1-x0,cs,0,LE)<dx/2 ) ad=99*ad/100;
        MSG(fprintf(stderr,"ad= %d",ad);)        
        // if( loop(box1->p,x0,y1-((dy>5)?1:0),x1-x0,cs,0,LE)>(dx+1)/2 )
        // if( loop(box1->p,x0,    y1         ,x1-x0,cs,0,LE)>(dx+1)/2 )
        //  { ad=96*ad/100; MSG(fprintf(stderr,"ad= %d",ad);) }
      }
      // if(box1->dots==1) { Setac(box1,';',ad); ad=99*ad/100; }
      Setac(box1,bc,ad);
   }
   // --- first test small dots ''""`` (quotation)--------------
   if (// 2*dy < box1->m4  -box1->m1+1 // failed for ocr-b dy=8 bad_m4-m1=13
         dy < box1->m3 - box1->m2  // should work always 2010-10-08
    && 2*y0 < box1->m2 + box1->m3
    && 3*y1 < box1->m2+2*box1->m3+2 ){
      ...
      if (box1->num_frames>1) { // ~!
        ...
        if (get_bw(x0,x1,(box1->m2+3*box1->m3)/4,box1->m4,box1->p,cs,1)!=0)
          { ad=98*ad/100; MSG({}) }
      }
      if (dx>4 
       && num_cross(x0,x1,(y0+3*y1)/4,(y0+3*y1)/4,box1->p,cs) == 2) { // " "
        DBG( c_ask='\"'; )
      ...
      if ( num_cross(x1,x1, y0      , y1      ,box1->p,cs)!=1
        && num_cross(x0,x0, y0      , y1      ,box1->p,cs)!=1) ad=99*ad/100;
      ...
      if ( loop(box1->p,x0,y0,dx,cs,0,RI)==0
        && loop(box1->p,x0,y1,dx,cs,0,RI)> 0
        && loop(box1->p,x1,y0,dx,cs,0,LE)> 0
        && loop(box1->p,x1,y1,dx,cs,0,LE)==0) bc='`'; // 2010-10-08 qemu
                  
      if (2*y0 > box1->m1+box1->m2) ad=99*ad/100;
      Setac(box1,bc,ad);
      if (ad>=100) return bc;
   }
   // --- first double dots ,, (quotation) -------------- 2010-10-01
   if( 2*dy <  box1->m4  -box1->m1+1
    && 2*y0 >= box1->m2  +box1->m3
    &&   y1 <= box1->m4+1
    && box1->num_frames   == 2  // ToDo: quotation without gap???
    && box1->num_boxes    == 2
    && box1->num_subboxes == 0
    && sdata.holes.num   == 0 ){
      DBG( wchar_t c_ask='"'; )
      ...
      if ( num_cross(x0,x1,(y0+3*y1)/4,(y0+3*y1)/4,box1->p,cs) != 2) ad=90*ad/100;
      if ( num_cross(x1,x1, y0      , y1      ,box1->p,cs)!=1) ad=99*ad/100;
      if ( num_cross(x0,x1, y0+dy/4 , y0+dy/4 ,box1->p,cs)>2) ad=97*ad/100;
      if ( num_cross(x0,x1, y1-dy/4 , y1-dy/4 ,box1->p,cs)>2) ad=97*ad/100; // * 5x8font
      if (2*y1 > box1->m3+box1->m4) ad=99*ad/100;
      Setac(box1, DOUBLE_LOW_9_QUOTATION_MARK, ad);
      if (ad>=100) return bc;
   }
   // --- TILDE ~ ---------------------------------
   ...
   if( 2*dy<box1->m4-box1->m1 && dx>=dy && dx>2 && dy>1
    && 2*y0<  box1->m1+box1->m2
    && 3*y1<2*box1->m2+box1->m3
    && box1->num_frames   == 1  // ToDo: 2010-10-01 what about \^O ???
    && box1->num_boxes    == 1
    && box1->num_subboxes == 0
    && sdata.holes.num    == 0 ){
      DBG( wchar_t c_ask='^'; )
...
//   if( dots==1 ){ um='\''; }
  ...
  um = box1->modifier;
   ...
   if( OCR_JOB->cfg.only_numbers ) return ocr0n(&sdata);
